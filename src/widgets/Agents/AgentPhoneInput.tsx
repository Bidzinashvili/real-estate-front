...
import {
  GEORGIAN_AGENT_PHONE_PREFIX,
  normalizeGeorgianAgentPhone,
} from "@/features/agents/normalizeAgentPhone";
import { cn } from "@/shared/lib/utils";

type AgentPhoneInputProps = {
  id?: string;
  name?: string;
  value: string;
...
const PREFIX_LENGTH = GEORGIAN_AGENT_PHONE_PREFIX.length;
...
export function AgentPhoneInput({
  id,
  name,
  value,
  onChange,
  ...
  className,
  disabled,
}: AgentPhoneInputProps) {
  const inputRef = useRef<HTMLInputElement | null>(null);
  const pendingCaretRef = useRef<number | null>(null);
  const displayValue = normalizeGeorgianAgentPhone(value);
  ...
  function handleChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const nextValue = normalizeGeorgianAgentPhone(rawValue);
